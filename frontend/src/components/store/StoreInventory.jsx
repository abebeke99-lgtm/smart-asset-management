...
export default function StoreInventory() {
  ...
  const loadInventory = async (page = 1, refresh = false) => {
    ...
    try {
      ...
      setFilterOptions(response.data?.filters || { categories: [], locations: [], conditions: [] });
  ...
  const openDetails = async (item) => {
    ...
    try {
      const response = await apiClient.get('/api/transactions', { params: { asset_id: item.asset_id } });
      setHistory(Array.isArray(response.data?.transactions) ? response.data.transactions : []);
    } catch (requestError) {
      setHistory([]);
    } finally {
  ...
  return <main className="store-inventory-page">
    ...
    <section className="inventory-panel"><div className="table-heading"><h2>{t.title}</h2><span>{t.showing} {pagination.total ? `${(pagination.page - 1) * pagination.pageSize + 1}-${Math.min(pagination.page * pagination.pageSize, pagination.total)} ${t.of || 'of'} ${pagination.total}` : '0'}</span></div>{items.length ? <div className="inventory-table-wrap"><table><thead><tr><th>{t.itemId}</th><th>{t.name}</th><th>{t.category}</th><th>{t.quantity}</th><th>{t.available}</th><th>{t.reserved}</th><th>{t.assigned}</th><th>{t.status}</th><th><MapPin size={14} /> {t.location}</th><th>{t.stock}</th><th>{t.actions}</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.asset_tag || item.asset_id || item.item_id || '-'}</td><td><strong>{item.name || '-'}</strong><small>{item.serial_number || ''}</small></td><td>{item.category || '-'}</td><td>{value(item, 'quantity')}</td><td>{value(item, 'availableQuantity', 'available_quantity')}</td><td>{value(item, 'reservedQuantity', 'reserved_quantity')}</td><td>{value(item, 'issuedQuantity', 'issued_quantity')}</td><td>{status(item) || '-'}</td><td>{item.location || '-'}</td><td><span className={`stock-badge ${stockStatus(item).toLowerCase().replace(' ', '-')}`}>{stockStatus(item)}</span></td><td><button className="detail-button" type="button" title={t.details} aria-label={t.details} onClick={() => openDetails(item)}><Eye size={16} /></button></td></tr>)}</tbody></table></div> : <div className="empty-inventory"><Boxes size={30} /><p>{filters.search || filters.status || filters.category || filters.location || filters.condition || filters.lowStockOnly ? t.filteredEmpty : t.empty}</p><button type="button" onClick={() => navigate('/store/receive')}><PackagePlus size={16} /> {t.addStock}</button></div>}<footer className="pagination"><button type="button" disabled={pagination.page <= 1} onClick={() => loadInventory(pagination.page - 1)}><ChevronLeft size={16} /> {t.previous}</button><span>{pagination.page} / {pagination.totalPages || 1}</span><button type="button" disabled={pagination.page >= pagination.totalPages} onClick={() => loadInventory(pagination.page + 1)}>{t.next} <ChevronRight size={16} /></button></footer></section>
    {selected && <div className="inventory-modal-backdrop" role="presentation" onClick={() => setSelected(null)}><section className="inventory-modal" role="dialog" aria-modal="true" aria-labelledby="inventory-details-title" onClick={(event) => event.stopPropagation()}><div className="modal-heading"><div><p className="inventory-eyebrow">Inventory Details</p><h2 id="inventory-details-title">{selected.name || '-'}</h2></div><button type="button" aria-label="Close" onClick={() => setSelected(null)}><X size={18} /></button></div><dl className="inventory-detail-grid">{[['Asset code', selected.asset_tag || selected.asset_id], ['Serial number', selected.serial_number], ['Category', selected.category], ['Description', selected.description], ['Location', selected.location], ['Department', selected.department], ['Quantity', value(selected, 'quantity')], ['Available', value(selected, 'availableQuantity', 'available_quantity')], ['Reserved', value(selected, 'reservedQuantity', 'reserved_quantity')], ['Condition', selected.condition], ['Status', status(selected)], ['Last updated', selected.last_updated ? new Date(selected.last_updated).toLocaleString() : '-']].map(([label, detail]) => <div key={label}><dt>{label}</dt><dd>{detail || '-'}</dd></div>)}</dl><h3><History size={17} /> Stock history</h3>{historyLoading ? <p>Loading history...</p> : history.length ? history.map((entry) => <div className="inventory-history-item" key={entry.id}><strong><BadgeCheck size={15} /> {String(entry.type || 'Movement').replace(/_/g, ' ')}</strong><span>{entry.quantity || 0} · {entry.createdAt ? new Date(entry.createdAt).toLocaleString() : '-'}</span></div>) : <p>No stock movements recorded.</p>}</section></div>}
  </main>;
}
