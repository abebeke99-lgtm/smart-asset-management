...
export default function StoreDashboard() {
  ...
  return <main className="store-dashboard">
    ...
    <div className="store-columns"><section className="store-section"><div className="section-heading"><h2>{t.inventoryHealth}</h2><BarChart3 size={20} /></div><div className="chart-wrap"><Bar data={chart} options={{ responsive: true, maintainAspectRatio: false, animation: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { precision: 0 } } } }} /></div></section><section className="store-section"><div className="section-heading"><h2>{t.today}</h2><History size={20} /></div><div className="activity-list">{[['receive', t.receive], ['issue', t.issue], ['return', t.return], ['transfer', t.transfer], ['adjustment', t.adjustment], ['verificationScans', t.scans]].map(([key, label]) => <div key={key}><span>{label}</span><strong>{data.today?.[key] || 0}</strong></div>)}</div></section></div>
