...
router.get('/health', ...requireAdmin, async (req, res, next) => { try { const data = await monitoring.getOverview(req.query); return res.json({ success: true, data: data.health || data }); } catch (error) { next(error); } });
router.get('/system-health', ...requireAdmin, async (req, res, next) => { try { const data = await monitoring.getOverview(req.query); return res.json({ success: true, data: data.overall || data.health || data }); } catch (error) { next(error); } });
...
router.get('/export', ...requireAdmin, async (req, res, next) => { try { const data = await monitoring.getOverview(req.query); const rows = [['Metric', 'Value'], ['Overall Status', data.overall.status], ['Database', data.database.status], ['Uptime Seconds', data.application.uptimeSeconds], ['Requests', data.api.requests], ['Failed Requests', data.api.errors], ['Average Response Time', data.api.responseTime], ['Failed Logins', data.security.failedLogins], ['Locked Accounts', data.security.lockedAccounts]]; res.setHeader('Content-Type', 'text/csv; charset=utf-8'); res.setHeader('Content-Disposition', 'attachment; filename="system-monitoring.csv"'); return res.send(rows.map((row) => row.map((value) => JSON.stringify(value ?? '')).join(',')).join('\n')); } catch (error) { next(error); } });
